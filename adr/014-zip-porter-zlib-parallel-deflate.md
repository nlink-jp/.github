# ADR-014: zip-porter Single-File Parallel Deflate on zlib
...
| Triggered by | ADR-013 shipped entry-level parallelism; a single large file — an LLM model archive, a disk image — still compresses on one core |

## Context

ADR-013 parallelised *across* entries. Within one entry, Apple's Compression
...
nothing from ADR-013 (incompressible model weights already divert to
...
3. **Entries above a size threshold compress as ~16 MB blocks in bounded
   ...
   in order, spill as ADR-013 already does). Peak memory stays around
...
The incompressibility probe (ADR-013) still runs first: model weights and
