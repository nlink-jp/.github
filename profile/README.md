...
| [zip-porter](https://github.com/nlink-jp/zip-porter) | Swift | Windows-safe ZIP creation/extraction for macOS (AppKit GUI + embedded CLI) — replaces MacWinZipper/The Unarchiver for ZIPs: junk files excluded, NFC UTF-8 names (no split dakuten) with CP932 legacy mode, AES-256/ZipCrypto passwords, CP932 auto-detection and AES decryption on extract, The Unarchiver-style extraction settings, zip-slip guard. Cross-verified against Info-ZIP/ditto/7-Zip and real-machine verified on Windows Explorer. darwin/arm64 |
