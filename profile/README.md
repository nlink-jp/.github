...
| [voice-studio-mcp](https://github.com/nlink-jp/voice-studio-mcp) | Go | MCP server giving AI agents local speech synthesis for radio-drama / audiobook production — AivisSpeech Engine backend, script JSONL batch synthesis with content-hash cache, pronunciation dictionaries, ffmpeg mastering (mp3/m4b chapters, loudnorm, credits), voice-model license review workflow, bundled radio-drama Claude Code skill |
