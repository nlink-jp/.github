...
| [lagent](https://github.com/nlink-jp/lagent) | Go | Interactive CLI agent runtime on a local LLM served over an OpenAI-compatible API (LM Studio, Ollama), for work that should not go to a cloud API — offline, confidential projects, no per-token spend. The same minimal, auditable loop as gem-agent (read, edit, shell, MCP, approval), defended by sandbox-exec containment plus operator approval; reads a project's AGENTS.md, CLAUDE.md and .mcp.json as they are |
...
| [stail](https://github.com/nlink-jp/stail) | Go | Read-only Slack tail — streams new channel messages in real time over Socket Mode like tail -f, shows the last N, or starts from an absolute timestamp, and exports full channel history to JSON in the shared export schema of the Slack tools, writing a subset of what scat and scli write |
