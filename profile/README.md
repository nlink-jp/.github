...
| [scat](https://github.com/nlink-jp/scat) | Go | General-purpose content poster for the terminal — sends arguments, files or stdin to Slack as messages, DMs or uploads, streams stdin continuously, and exports a channel’s history to structured JSON. Multiple destination profiles, plus a server mode configured entirely by environment variables for containers and CI |
...
| [stail](https://github.com/nlink-jp/stail) | Go | Read-only Slack tail — streams new channel messages in real time over Socket Mode like tail -f, shows the last N, or starts from an absolute timestamp, and exports full channel history to JSON in scat’s export format. The reading half of scat |
...
| Tool | Lang | Description |
|------|------|-------------|
| [lagent](https://github.com/nlink-jp/lagent) | Go | Sandboxed CLI agent runtime on a local LLM served over an OpenAI-compatible API — file read/write, sandboxed shell and MCP servers, with mutating calls gated by the operator. Same minimal auditable loop as gem-agent, built to measure how far a local model carries an agent runtime |
...
| [spice-client](https://github.com/nlink-jp/spice-client) | Swift | Native SPICE client for QEMU and Ravada virtual desktops — opens .vv files and an HTTPS Ravada portal, and confirms host, port, TLS and clipboard (off by default) natively before connecting. One window per session, text clipboard only for the focused window, files sent to the guest by drop or menu; SwiftSpice inside, for Apple Silicon |
...
| [agent-board](https://github.com/nlink-jp/agent-board) | Go | Machine-local shared knowledge board for concurrent AI agent sessions — append-only records posted as claims and delivered as per-turn deltas through Claude Code and gem-agent hooks, with path claims that refuse another session’s edits. A write-side evaluation ladder stands in for a human gate |
...
| [chrome-pilot-mcp](https://github.com/nlink-jp/chrome-pilot-mcp) | Go | Browser automation as an MCP server with no supply chain to trust — a single binary with zero external modules speaking CDP directly, driving your installed Chrome, with startup-only host allow/block lists no tool can widen |
...
| [net-meter](https://github.com/nlink-jp/net-meter) | Swift | macOS menu-bar meter for one network interface — the current upstream and downstream rate as two lines of numbers and a graph that scrolls one bar a second, with a panel for a three-minute history, addresses, link speed, peaks and totals. Follows the physical link even while a VPN is up. No permissions, no network connections of its own, nothing else. Swift/AppKit + SwiftUI, macOS 26+, darwin/arm64 |
