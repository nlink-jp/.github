...
| [video-studio-mcp](https://github.com/nlink-jp/video-studio-mcp) | Go | MCP server that assembles a narrated presentation video (MP4) from a page manifest (image + audio per page) — a pure ffmpeg compositor paired with voice-studio-mcp; per-page audio sets each page's duration (exact A/V sync), per-page chapter markers, opt-in burned-in captions (bundled M PLUS 1p, Go-rendered overlay — no libfreetype needed), per-call canvas override (16:9/9:16/1:1), async rendering with check_job |
