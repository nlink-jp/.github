...
| [brave-search](https://github.com/nlink-jp/brave-search) | Go | Brave Search API as a CLI and local MCP server — Web Search (ranked results with snippets), LLM Context (page text pre-extracted to a token budget, for grounding a model) and Answers (a Brave-grounded answer with citations, single-search or multi-step research). A search primitive that needs only Brave API keys, no GCP: it returns what Brave returns, prints what each call cost and the rate budget left, and — by the Brave ToS — caches nothing and stores nothing. gem-search is the agentic report generator; this is the search call |
...
| [task-clock-gui](https://github.com/nlink-jp/task-clock-gui) | Swift | Menu-bar front end for task-clock — a quiet-by-default clock that speaks only when it matters (overrun badge with the overrun duration, a distinct question-mark state when the daemon is unreachable); pilot-lamp daemon control (lamp = actual state, power switch = run state via the bundled CLI's `start`/`stop` — stopping never kills running tasks — with install/uninstall as a separate setup control and Restart for a broken one), a user-resizable panel, per-task rows with per-second countdowns, run-now behind a two-click interlock and persistent on/off switches, a per-task scheduled-vs-actual run-history view (one line per fire with start delay, duration and result, each with its captured log one click away), notifications for overrun entry / run failure / daemon-down (deliberate stops stay silent), launch-at-login; bundles the signed task-clock CLI as its only data path. macOS 14+, darwin/arm64 |
...
| [web-fetch](https://github.com/nlink-jp/web-fetch) | Go | One URL's readable text, fetched from this machine, as a CLI and local MCP server — the page's main content as markdown (or plain text, or the raw body), paged by `offset` with a `doc_id` that tells a changed page from a page turn, in any declared charset (Shift_JIS and EUC-JP included). A primitive for agent runtimes that can search but cannot read a user-given URL: no cookies, no Authorization, no proxy, nothing on disk, and a finite denied-address guard applied inside the HTTP dialer on the addresses a name actually resolves to — loopback, private, link-local and cloud-metadata ranges are never reached, on any redirect hop |
