...
| モデル名 | サイズ | VRAM目安 | 備考 |
|---------|-------|---------|------|
| `google/gemma-4-26b-a4b` | ~18 GB | 20-24 GB | **nlink-jp 標準モデル。動作確認済み** |

> nlink-jp のツールは **gemma-4-26b-a4b**（Gemma 4 27B Instruct、MoEで実効4Bパラメータ動作）を標準モデルとして開発・テストしています。他のモデルでも動作する可能性はありますが、動作確認はこのモデルで行っています。
>
> 検索バーに `gemma-4-26b-a4b` と入力し、GGUF 形式の量子化版（Q4_K_M 等）を選択してください。VRAM が不足する場合は、LM Studio が自動的に CPU とのハイブリッド実行を行います。
